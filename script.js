...
form.addEventListener("submit", async e => {
  ...
  if(err){msg.textContent=err;msg.className="msg show error";shakeInputs();return;}
  ...
  try{
    ...
    if(res.ok){
      ...
      form.reset();
      setTimeout(()=>{success.classList.add("fade-out");setTimeout(()=>location.href="index.html",1000);},3000);
    } else throw 0;
  }catch{
...
function shakeInputs(){
  ...
  inputs.forEach(inp=>{inp.classList.add("shake");setTimeout(()=>inp.classList.remove("shake"),400);});
}