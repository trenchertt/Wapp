(function(){
  ...
  f.addEventListener("submit", async function(t){
    ...
    if(!n || !a || !i){
      e.textContent="Please fill in all fields.";
      e.classList.add("show");
      return;
    }
    if(!re.test(a)){
      e.textContent="Enter a valid e-mail.";
      e.classList.add("show");
      return;
    }
    if(!rp.test(i)){
      e.textContent="Password must be 6-21 chars, include letters, numbers & special symbol.";
      e.classList.add("show");
      return;
    }
    ...
    try{
      ...
      e.textContent=(d&&(d.error||d.message))?(d.error||d.message):"Something went wrong. Try again.";
      e.classList.add("show");
    }catch(o){
